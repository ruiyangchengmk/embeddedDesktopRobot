...
| 显示屏 | LCD1602A + PCF8574T 背板 | 本地状态显示（角度、RGB 值） |
...
| **EC11 VCC** | 红 | **3.3V** | - | - |
| **EC11 GND** | 黑 | **GND** | - | - |
| **LCD1602 SDA** | I2C 数据 | **GPIO8** | 5V 供电 | PCF8574 I2C 背板 |
...
| 任务名 | 优先级 | 周期 | 职责 |
|--------|--------|------|------|
| `ec11_task` | 10 (高) | 事件驱动 | EC11 ISR 队列 → 更新舵机 + RGB |
| `app_main` | 1 (低) | 50ms | LCD 刷新（调用 `hal_ec11_get_angle()`） |
| `gpio_isr_handler` | ISR | 中断驱动 | EC11 旋转/按键检测，消抖，队列发送 |
...
> **为什么必须用队列？**
> `hal_rgb_set_color` 内部调用 `rmt_tx_wait_all_done`，会等待 FreeRTOS 信号量。此操作在 ISR 内会导致 `Interrupt wdt timeout` panic。因此 ISR 只负责把事件入队，实际控制操作全部在 `ec11_task` 内完成。
