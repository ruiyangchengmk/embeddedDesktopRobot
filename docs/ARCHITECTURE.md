...
### 1.1 一句话定位
一个以 **ESP32-S3-DevKitC-1** 为核心的端侧控制节点。当前稳定固件以**本地交互链路**为主：

- `EC11` 提供旋转与按键输入
- `event_broker` 负责本地事件广播
- `SG90`、`WS2812`、`GC9A01` 在独立任务中消费事件

WiFi + MQTT 相关代码仍保留在仓库中，但不是当前默认运行路径。
...
### 1.3 当前进度
- ✅ **本地控制链路稳定**：`EC11 → SG90 / RGB / GC9A01`
- ✅ **显示启动链路稳定**：`hal_gc9a01_init()` 后再启动 `lvgl_task`
- ✅ **编译 / 烧录 / monitor 已实机验证**
- ⏳ **网络层保留待回接**：WiFi + MQTT 代码在仓库中，但当前默认未启用
...
```
/home/mikurubeam/Desktop/espattack/   # 项目根目录
...
└── main/                             # 主组件（ESP-IDF 应用入口）
    ...
    │   └── hal_hcsr04.h / .c        # HC-SR04 超声波测距驱动
...
| 显示屏 | GC9A01 1.28" 圆形 SPI LCD | 本地状态显示（表情图，EC11 按键切换） |
| 超声波测距 | HC-SR04 (4 PIN) | 实时距离测量，屏幕标签 + 串口持续输出 |
...
| **EC11 VCC** | 红 | **3.3V** | - | - |
| **EC11 GND** | 黑 | **GND** | - | - |
| **HC-SR04 VCC** | 红 | **5V** | USB 供电可直接驱动 | - |
| **HC-SR04 GND** | 黑 | **GND** | - | - |
| **HC-SR04 Trig** | 黄 | **GPIO8** | 发送脉冲 | - |
| **HC-SR04 Echo** | 白 | **GPIO9** | 接收反射 | - |
| **I2S BCLK** | BCLK | **GPIO13** | 模块 VCC 支持 3.3V-5V | 功放位时钟 |
| **I2S LRCLK** | LRCLK / WS | **GPIO14** | - | 左右声道选择时钟 |
| **I2S DOUT** | SDA | **GPIO15** | - | ESP32-S3 输出到功放 |
| **I2S DIN** | DATA | **GPIO16** | - | 麦克风输入预留，当前未启用 |
| **I2S MCLK** | CLK | **GPIO17** | 可选 | 当前固件默认不驱动，用于兼容需要 MCLK 的模块 |
...
**底层驱动**：ESP-IDF `GPIO` + 任务轮询 + 按键中断 (`gpio_isr_service`)
**默认 GPIO**：`CLK=GPIO5`, `DT=GPIO6`, `SW=GPIO7`

> **重要**：旋转检测已经迁移到任务轮询；ISR 仅用于 SW 按键。ISR 内**禁止调用阻塞 API**。所有实际控制操作都通过事件总线和任务上下文完成。
...
QueueHandle_t hal_ec11_get_queue(void);  // 获取 EC11 私有事件队列，元素类型为 hal_ec11_msg_t
...
**实现细节**：
- 旋转检测：`ec11_reader_task` 以 `2ms` 周期轮询 `CLK/DT`
- 方向判断：检测到 `CLK` 从高到低时读取 `DT` 电平
  - `DT == 1` → 顺时针 (CW)
  - `DT == 0` → 逆时针 (CCW)
- 按键检测：`SW` 使用 GPIO 下降沿中断 + `200ms` 消抖
- `hal_ec11_get_queue()` 返回独立 EC11 事件队列；同时事件仍会发布到 `event_broker`
- 角度范围：硬限制在 `0° ~ 180°`，两端限位阻断（需反向旋转解除）
- 旋转处理在任务上下文中完成；按键 ISR 只做消抖 + 广播，不做阻塞操作

---

### 4.5 超声波测距模块：`hal_hcsr04`

**文件**：`main/hal/hal_hcsr04.h`, `main/hal/hal_hcsr04.c`
**底层驱动**：ESP-IDF `GPIO` + busy-wait 精确延时
**默认 GPIO**：`Trig=GPIO8`, `Echo=GPIO9`

**接口**：
```c
hal_err_t hal_hcsr04_init(void);
hal_err_t hal_hcsr04_deinit(void);
float     hal_hcsr04_get_distance_cm(void);  // 阻塞式，返回 cm 或 -1（超时/无效）
```

**实现细节**：
- 测量原理：Trig 发送 10μs 脉冲 → Echo 高电平宽度 = 往返时间 → `distance = time_us × 0.01715`
- Trig 脉冲：`gpio_set_level(GPIO8, 1)` → `ets_delay_us(10)` → `gpio_set_level(GPIO8, 0)`
- Echo 检测：纯 busy-wait `gpio_get_level()` 循环，`HCSR04_MAX_ITER = 30000`（~30ms 超时，对应 ~500cm）
- 测量范围：2cm ~ 400cm，超出返回 -1
- 测量频率：建议间隔 ≥ 60ms（60ms = 300cm 上限对应的回波时间）

> **Core 亲和性**：`hcsr04_task` 必须运行在 **core 1**，避免与 GC9A01 SPI 总线争抢导致任务饿死。
...
- 断线自动重连，最多重试 10 次。
- `net_wifi_init()` 内部会负责 `nvs_flash_init()`，并处理 `ESP_ERR_NVS_NO_FREE_PAGES / ESP_ERR_NVS_NEW_VERSION_FOUND`。
...
- 只接受与 `TOPIC_COMMANDS` **完全相等**的 topic，避免前缀误匹配。
- 解析结果必须是 `0~180` 的整数；未命中或越界会直接丢弃并记录告警。
...
3. 启动 `consumer_task`、`servo_task`、`rgb_task`
4. 初始化 `GC9A01`，再启动 `lvgl_task`
5. 最后启动 `EC11`
6. 所有任务创建都检查返回值；`GC9A01` 初始化失败时不会继续启动 `lvgl_task`
...
| 任务名 | 优先级 | 周期 | 职责 |
|--------|--------|------|------|
| `ec11_reader` | **12** | 2ms 轮询 | 只读 EC11 角度，立即广播到 `event_broker` |
| `consumer_task` | **18** | 事件驱动 | 订阅事件总线，将 `broker_event_t` 转换为控制消息并广播到 4 个私有队列 |
| `servo_task` | 15 | 每 10ms | 只保留最新目标值并自适应步进 |
| `rgb_task` | 14 | 事件驱动 | 只保留最新颜色目标并立即更新 RGB |
| `hcsr04_task` | 10 | 5Hz（200ms） | **core 1**，测量距离并通过 `event_broker` 发布 `EVENT_TYPE_HCSR04_DISTANCE` |
| `audio_task` | 7 | 事件驱动 | 初始化 I2S TX，在距离小于 10cm 时播报警告语音 |
| `lvgl_task` | 5 | LVGL 定时器驱动 | 订阅 `s_hcsr04_queue`，更新距离标签；EC11 按键切换表情图 |

**设计理由：**
- EC11 读取是最高优先级，任何时候都不能被延迟
- `consumer_task` 将事件总线消息转换为统一的控制消息，并分别发送到 4 个**私有队列**（`s_servo_queue`、`s_rgb_queue`、`s_lvgl_queue`、`s_audio_queue`），彻底避免共享队列的竞态
- `s_servo_queue`、`s_rgb_queue`、`s_lvgl_queue` 是单槽覆盖队列，消费者只保留最新目标值；`s_audio_queue` 是单槽非覆盖队列，正在播放时不抢占当前语音
...
```
EC11 旋转/按键 → hal_ec11.c: ec11_reader_task（2ms 轮询）/ SW ISR
               ...
               → consumer_task: xQueueReceive(s_consumer_queue)
                        ...
                        → 广播到 4 个私有队列：
                             ...
                             xQueueSend(s_audio_queue, &warning_req, 0)

HC-SR04 测距 → hal_hcsr04.c: hcsr04_task（core 1，5Hz）
            → event_broker_publish(EVENT_TYPE_HCSR04_DISTANCE, dist_mm)
            → consumer_task: 缓存 latest_distance_cm_x10
            → lvgl_task: xQueueReceive(s_hcsr04_queue)
                        → lv_label_set_text(s_dist_label, ...)
                        → lv_obj_invalidate(s_dist_label)

私有队列分别消费：
  → servo_task（优先级 15）：只跟踪最新目标，按误差自适应步进
  → rgb_task（优先级 14）：直接更新颜色，无延迟
  → audio_task（优先级 7）：距离小于 10cm 时完整播报固定中文告警，3 秒内最多一次
  → lvgl_task（优先级 5）：EC11 按下时切换表情图；实时更新距离标签
```
...
> **事件总线节流现在是什么？**
> `event_broker_publish()` 现在使用 `1ms burst guard` 过滤极端机械抖动，不是历史文档里曾出现过的“100Hz 节流”。
...
export IDF_PATH=/home/mikurubeam/.espressif/v6.0/esp-idf
# 若 idf.py 环境正常
idf.py build
...
以添加 "超声波测距模块 HC-SR04" 为例（已实现，可直接参考）：
...
hal_err_t hal_hcsr04_init(void);
hal_err_t hal_hcsr04_deinit(void);
float     hal_hcsr04_get_distance_cm(void);  // 阻塞式，返回 cm 或 -1

#endif
```

**Step 2**：在 `hal_hcsr04.c` 中实现驱动，仅依赖 `driver/gpio.h` 和 `rom/ets_sys.h`，不引用任何应用层变量。使用 busy-wait 精确定时，不要依赖 `esp_timer`（可能被 SPI 总线阻塞）。

**Step 3**：`main/CMakeLists.txt` **无需修改**（已使用 `GLOB_RECURSE` 自动收集）。

**Step 4**：在 `main.c` 的 `app_main()` 中创建独立任务并绑定 **core 1**：
```c
// app_main() 中
...
static void hcsr04_task(void *arg)
{
    ...
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(200));  // 5Hz，避免与 SPI 总线争抢
    }
}
```

> **关键**：传感器任务必须放在 **core 1**，避免与 GC9A01 SPI 总线争抢导致任务饿死。
...
| 2026-05-08 | v0.12 | **HC-SR04 超声波测距**：新增 `hal_hcsr04.h/.c`，GPIO8=Trig/GPIO9=Echo，busy-wait 精确定时；`hcsr04_task` 运行于 **core 1**（避免 SPI 总线争抢导致任务饿死）；`EVENT_TYPE_HCSR04_DISTANCE` 通过 `event_broker` 广播；距离标签显示于 GC9A01 屏幕顶部中央（青色文字 + 黑色不透明背景），lvgl 每秒更新多次。 |
