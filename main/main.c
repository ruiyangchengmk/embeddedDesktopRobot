/**
 * main.c — ESP32-S3 端侧控制节点 (v0.7)
 *
 * 架构：EC11 GPIO 中断 → 事件总线 → 各消费者独立任务
 * 原则：单一硬件变更不影响其他硬件；分层架构最小修改。
 *
 * 任务优先级（从高到低）：
 *   ec11_isr     ISR  — GPIO 下降沿中断，发布旋转/按键事件
 *   consumer     18  — 事件总线消费者，转换为 angle_msg
...
// 消费者队列（各任务私有，避免互斥）
// ================================================================
static QueueHandle_t s_angle_queue = NULL;  // 广播给所有消费者的队列
...
// ================================================================
// 事件总线消费者任务（替代原来的 ec11_reader_task）
// ================================================================
...
static void rgb_task(void *arg)
{
    ...
    while (1) {
        angle_msg_t msg;
        if (s_angle_queue != NULL &&
            xQueueReceive(s_angle_queue, &msg, pdMS_TO_TICKS(100)) == pdTRUE) {
            if (!msg.button) {
...
static void lvgl_task(void *arg)
{
    ...
    while (1) {
        angle_msg_t msg;
        if (s_angle_queue != NULL &&
            xQueueReceive(s_angle_queue, &msg, 0) == pdTRUE) {
...
{
    // ---- 第一步：创建所有队列（在任何任务启动前）----
    s_angle_queue = xQueueCreate(10, sizeof(angle_msg_t));
    if (!s_angle_queue) {
        ESP_LOGE(TAG, "s_angle_queue create failed");
        return;
    }

    s_consumer_queue = xQueueCreate(10, sizeof(broker_event_t));
    if (!s_consumer_queue) {
        ESP_LOGE(TAG, "s_consumer_queue create failed");
        return;
    }
    ...
    ESP_LOGI(TAG, "System initialized — event_broker + pcnt_encoder");
}
